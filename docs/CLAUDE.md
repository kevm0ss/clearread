...
readclear reformats web pages, documents, and scanned letters to be easier to read for people with dyslexia. **Content is never reduced. Only reorganised.**
...
| File | What it covers |
|---|---|
| [architecture.md](architecture.md) | Pages, Worker endpoints, deploy pipeline, data flow |
| [stack.md](stack.md) | Tools, accounts, models, why each was chosen |
| [profiles.md](profiles.md) | The four dyslexia profiles, traits, quiz logic |
| [profile-storage.md](profile-storage.md) | **Critical** — how profile is saved/read in localStorage across pages |
| [worker.md](worker.md) | Cloudflare Worker — endpoints, CORS, prompts, model |
...
4. **Flag conflicts** before implementing — don't silently override an architectural decision
...
- Content is never removed or simplified — only restructured
...
- Carry the dyslexia profile across: read with `getProfile()`, save with `setProfile()`
